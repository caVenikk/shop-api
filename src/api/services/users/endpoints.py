...
@router.get(
    "/", status_code=status.HTTP_200_OK, responses={200: {"model": list[sc.ResponseUser], "description": "Users array"}}
)
...
@router.get(
    ...
    responses={200: {"model": sc.ResponseUser, "description": "User"}, 404: {"description": "User not found"}},
)
...
@router.post(
    ...
    responses={201: {"model": sc.ResponseUser, "description": "User successfully created"}},
)
...
@router.put(
    ...
    status_code=status.HTTP_200_OK,
    responses={
        200: {"model": sc.ResponseUser, "description": "User successfully updated"},
        404: {"description": "User not found"},
    },
)
async def update_user(new_user: sc.User, crud: CRUD = Depends(CRUD)):
    updated_user = md.User(**new_user.dict())
    if not await crud.users.update(user=updated_user):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with id={new_user.id} not found.")
...
@router.delete(
    "/{user_id}",
    status_code=status.HTTP_200_OK,
    responses={200: {"description": "User now inactive"}, 404: {"description": "User not found"}},
)
async def delete_user(user_id: int, crud: CRUD = Depends(CRUD)):
    if not await crud.users.delete(user_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with id={user_id} not found.")
    return JSONResponse(content={"detail": f"User with id={user_id} now inactive."})


@router.patch(
    "/{user_id}/restore",
    status_code=status.HTTP_200_OK,
    responses={200: {"description": "User restored"}, 404: {"description": "User not found"}},
)
async def restore_user(user_id: int, crud: CRUD = Depends(CRUD)):
    if not await crud.users.restore(user_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User with id={user_id} not found.")
    return JSONResponse(content={"detail": f"Restored user with id={user_id}."})
